...
if [ "$VERSION" = "latest" ]; then
    VERSION=$(curl --proto '=https' --tlsv1.2 -sL https://api.github.com/repos/gravitational/teleport/releases/latest \
        | grep '"tag_name"' \
        | sed 's/.*"tag_name": *"v\([^"]*\)".*/\1/')
fi
...
curl --proto '=https' --tlsv1.2 -fL -o "$TARBALL" "${BASE_URL}/${TARBALL}"
curl --proto '=https' --tlsv1.2 -fL -o "${TARBALL}.sha256" "${BASE_URL}/${TARBALL}.sha256"
